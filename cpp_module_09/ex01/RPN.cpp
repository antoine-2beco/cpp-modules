...
/*   Updated: 2025/09/03 12:16:07 by ade-beco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     RPN::calculate( char sign ) {
    ...
    if ( result > INT_MAX || result < INT_MIN ) {
        std::cerr << "Error : Maximum amount reached" << std::endl;
        exit (EXIT_FAILURE);
    }
    return result;
}
...
void     RPN::expressionProcess( const std::string &expression ) {
    for (unsigned long i = 0; i != expression.length(); i++) {
        if (expression[i] == ' ')
            ...
            if  (isOperator(expression[i]) == TRUE)
                _expressionStack.push(calculate(expression[i]));
            else if (isDigit(expression[i]) == TRUE)
                _expressionStack.push(expression[i] - '0');
    ...
    std::cout << _expressionStack.top() << std::endl;
    _expressionStack.pop();
}
